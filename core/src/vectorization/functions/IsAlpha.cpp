 ...
 *
 * Unicode letters are recognized via an embedded sorted range table generated
 * from the Unicode 15.0 character database (categories Lu/Ll/Lt/Lm/Lo). This
 * matches Java's Character.isLetter for the vast majority of code points; only
 * code points whose letter classification differs between Unicode versions may
 * diverge from a specific JDK build.
 */

#include "vectorization/functions/IsAlpha.h"
...
// Sorted, non-overlapping ranges of Unicode code points classified as letters
// (general categories Lu, Ll, Lt, Lm, Lo) per Unicode 15.0.
...
constexpr size_t kLetterRangeCount = sizeof(kLetterRanges) / sizeof(kLetterRanges[0]);

/// Returns true iff codePoint is a Unicode letter (binary search over kLetterRanges).
ALWAYS_INLINE bool IsUnicodeLetter(int32_t codePoint)
{
    if (codePoint < 0) {
        return false;
    }
    ...
    size_t lo = 0;
    size_t hi = kLetterRangeCount;  // [lo, hi)
    while (lo < hi) {
        size_t mid = lo + ((hi - lo) >> 1);
        if (cp < kLetterRanges[mid].lo) {
            hi = mid;
        } else if (cp > kLetterRanges[mid].hi) {
            lo = mid + 1;
        } else {
            return true;
        }
...
/// Decodes the first UTF-8 code point in [data, data+size). Sets byteLen to the
/// number of bytes consumed. Returns -1 on invalid/incomplete UTF-8 (byteLen=1).
/// Mirrors vectorization/functions/String.h::Utf8FirstCodepoint (kept local to
/// avoid pulling that header's heavy dependencies).
ALWAYS_INLINE int32_t DecodeUtf8(const char *data, size_t size, int &byteLen)
{
    ...
    if (u0 >= 0xC0 && u0 <= 0xDF) {
        byteLen = 2;
        // Reject overlong 2-byte forms (0xC0/0xC1 encode code points < 0x80). RFC 3629.
        if (u0 < 0xC2) {
            return -1;
        }
        if ((u1 & 0xC0) != 0x80) {
    ...
    if (u0 >= 0xE0 && u0 <= 0xEF) {
        ...
        // Reject overlong 3-byte forms (0xE0 with u1 < 0xA0 encode code points < 0x800). RFC 3629.
        if (u0 == 0xE0 && u1 < 0xA0) {
            return -1;
        }
        return (u0 - 0xE0) * 4096 + (u1 - 0x80) * 64 + (u2 - 0x80);
    }
    if (size < 4) {
        byteLen = 1;
        return -1;
    ...
    if (u0 >= 0xF0 && u0 <= 0xF7) {
        ...
        // Reject overlong 4-byte forms (0xF0 with u1 < 0x90 encode code points < 0x10000). RFC 3629.
        if (u0 == 0xF0 && u1 < 0x90) {
            return -1;
        }
        return (u0 - 0xF0) * 262144 + (u1 - 0x80) * 4096 + (u2 - 0x80) * 64 + (u3 - 0x80);
    }
    byteLen = 1;
    return -1;  // invalid lead byte
}

/// Returns true iff typeId is a numeric type accepted by Flink's IS_ALPHA
/// (OperandTypes.NUMERIC). For numeric input Flink's isAlpha returns false
/// (the value is not a String instance).
...
/// Returns true iff the string is non-empty and every code point is a Unicode
/// letter. Any invalid UTF-8 sequence makes the whole string non-alpha.
bool IsAllUnicodeAlpha(const std::string_view &s)
{
    size_t i = 0;
    size_t n = s.size();
    while (i < n) {
        int byteLen = 1;
        int32_t cp = DecodeUtf8(s.data() + i, n - i, byteLen);
        if (cp < 0 || !IsUnicodeLetter(cp)) {
            return false;
        }
...
class IsAlphaFunction : public VectorFunction {
public:
    ...
    {
        if (args.empty()) {
            OMNI_THROW("IS_ALPHA Error", "IS_ALPHA requires 1 argument; got 0");
        }
        ...
        // Flink semantics: numeric input -> always false (isAlpha: not a String instance).
        // NULL input -> false (output NOT null). Output is never NULL.
        const bool numericInput = IsNumericType(inputVec->GetTypeId());

        for (int32_t row = 0; row < rowSize; ++row) {
            bool value = false;
            if (!numericInput && !inputVec->IsNull(row)) {
                std::string_view s = GetStringValueFromVector(inputVec, row);
                // Flink semantics: empty string -> false
                value = !s.empty() && IsAllUnicodeAlpha(s);
            }
...
private:
    ...
    static std::string_view GetStringValueFromVector(BaseVector *vec, int32_t row)
    {
        ...
        OMNI_THROW("IS_ALPHA Error", "Unsupported encoding type for string input");
    }
};
...
{
    auto isAlphaFunction = std::make_shared<IsAlphaFunction>();
    // String input types (CHAR/VARCHAR): evaluate per code point.
    VectorFunction::RegisterVectorFunction(name, {OMNI_VARCHAR}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_CHAR}, OMNI_BOOLEAN, isAlphaFunction);
    // Numeric input types (Flink OperandTypes.NUMERIC): always return false.
    VectorFunction::RegisterVectorFunction(name, {OMNI_BYTE}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_SHORT}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_INT}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_LONG}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_FLOAT}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_DOUBLE}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_DECIMAL64}, OMNI_BOOLEAN, isAlphaFunction);
    VectorFunction::RegisterVectorFunction(name, {OMNI_DECIMAL128}, OMNI_BOOLEAN, isAlphaFunction);
}
