 ...
 * Description: flink_dayofweek function implementation
 *
 * flink_dayofweek(date) -> int32, flink_dayofweek(timestamp) -> int32
 *
 * Mirrors Flink's DAYOFWEEK(date) == EXTRACT(DOW FROM date) semantics:
 *   - OMNI_INT  : date, interpreted as days since epoch (1970-01-01). The day
 *                 of week is extracted in UTC via the Gregorian decomposition,
 *                 exactly like Flink's extractFromDate(DOW, days).
 ...
 *                 TIMESTAMP inlines extractFromDate(DOW, millis / 86400000),
 ...
 *
 * DOW formula: Flink julianExtract DOW = floorMod((julian + 1), 7) + 1,
 * yielding Sunday=1, Monday=2, ..., Saturday=7 (Spark/Hive convention).
 * NOTE: this is the DOW unit, NOT ISODOW (which is floorMod(julian, 7) + 1,
 * Monday=1..Sunday=7). DAYOFWEEK maps to DOW.
 * With std::tm.tm_wday in [0..6] (Sunday=0, Monday=1, ..., Saturday=6) this
 * is exactly `tm_wday + 1`, the form used here (and by the existing
 * DayOfWeek.cpp).
 *
...
/// flink_dayofweek function
/// flink_dayofweek(date) -> int32, flink_dayofweek(timestamp_millis) -> int32
/// OMNI_INT  = days since epoch (date); day of week extracted in UTC.
/// OMNI_LONG = milliseconds since epoch (Flink TimestampData); day of week
///             extracted in UTC (no session timezone — wall-clock semantics,
///             matching Flink EXTRACT(DOW FROM <TIMESTAMP>)).
/// Returns Sunday=1..Saturday=7 (DOW, not ISODOW).
/// Returns NULL if the input is NULL (or out of range for OMNI_INT).
class FlinkDayOfWeekFunction : public VectorFunction {
public:
        ...
        op::ExecutionContext *context) const override
    {
        ...
        if (inputTypeId == OMNI_INT) {
            // date = days since epoch; extract day of week in UTC (no timezone for DATE).
            ...
            rows.applyToSelected([&](vector_size_t i) {
                ...
                if (Timestamp::epochToCalendarUtc(seconds, tmValue)) {
                    // tm_wday is 0-6 (Sunday=0); +1 gives Sunday=1..Saturday=7,
                    // equal to Flink's floorMod((julian + 1), 7) + 1 (DOW, not ISODOW).
            ...
            // extractFromDate(DOW, millis / 86400000).
...
void RegisterFlinkDayOfWeekFunction(const std::string &name)
{
    ...
    // millis since epoch) are supported, per the Flink DAYOFWEEK semantics this
    ...
    VectorFunction::RegisterVectorFunction(name, {OMNI_INT}, OMNI_INT,
        std::make_shared<FlinkDayOfWeekFunction>());
